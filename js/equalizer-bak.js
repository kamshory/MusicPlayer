...
function markListItem(list, target)
{
	var i, j;
	for(var i = 0; i < list.length; i++)
	{
		list[i].className = 'music-item';
	}
	target.className = 'music-item-selected';
}
...
window.onload = function()
{
	...
	var list = document.querySelectorAll('[data-action="play-item"]');
	var i, j;
	for(var i = 0; i < list.length; i++)
	{
		list[i].addEventListener('click', function(e){
			loadMusic(e.target.getAttribute('data-path'));
			markListItem(list, e.target);
			e.preventDefault();
		});		
	}
}
